...
Once the data is downloaded to a .SAV file, it is easy to use it using R by using the 'haven' package.
...
Now I turn the .SAV file that is saved in my path into a .Rda file.
...
V168 F SCDRY SCH ENROL GER 1980 ; 
...
We still have 148 rows but now only 10 columns are left. Let's look at the descriptive statistics for V14.
...
We see that there are a number of cases that are missing for V5. So we are going to remove the missing cases from not only V5, but the rest of the data frame as well. In statistics, this methods of dealing with missing data is called listiwse deletion.
...
Maybe I don't want to use V5 at all. How do I delete a column? I use the dplyr function select again.
...
We can see that a lot of countries have an infant mortality rate that is above 50 per 1000 births. Now what if I want to arrange the data according to infant mortality rate? I can use the 'arrange()' function.
...
Out of the countries with more than 50 infant deaths per 1000 live births, we see that Peru is the country with the lowest infant mortality rate and that Mozambique is the country with the highest infant mortality rate. If I wnted to arrange this data in descending order, I can use the code bellow.
...
Let's look at female school enrollment based on our new data frame.
